...
function solution(numbers) {
    ...
    for(let i = 0; i < numbers.length - 2; i++) {
        let tempTriple = [numbers[i], numbers[i + 1], numbers[i + 2]];
        res.push(isZigzag(tempTriple) ? 1 : 0);
    }

    return res;
}

function isZigzag(triple) {
    if((triple[1] < triple[0] && triple[1] < triple[2]) || (triple[1] > triple[0] && triple[1] > triple[2])) {
        return true;
    }
